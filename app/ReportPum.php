...
class ReportPum extends Model
{
    ...
    public function permohonanPumGroupDept($user_id, $create_start_date, $create_end_date, $pum_status, $resp_status, $validate_start_date, $validate_end_date, $org_id){
        $getEmp = DB::connection('api_pum')->table('pum_trx_all as a')
            ->selectRaw("distinct b.NAME as DEPT_NAME, a.DEPT_ID, '' as dataReport")
            ->leftJoin('api_hr.hr_departments as b', 'b.DEPT_ID', 'a.DEPT_ID')
            ->where('a.CREATED_BY', $user_id)
            ->get()->toArray();
    ...
    public function permohonanPumGroupDate($user_id, $create_start_date, $create_end_date, $pum_status, $resp_status, $validate_start_date, $validate_end_date, $org_id){
        $getEmp = DB::connection('api_pum')->table('pum_trx_all as a')
            ->selectRaw("distinct a.TRX_DATE, '' as dataReport")
            ->where('a.CREATED_BY', $user_id)
            ->get()->toArray();
        ...
        foreach ($getEmp as $data){
            $search   = DB::connection('api_pum')->table('pum_trx_all as a')
                ...
                ->leftJoin('api_hr.hr_employees as c', 'c.EMP_ID', 'a.EMP_ID')
                ->leftJoin('pum_trx_lines_all as d', 'd.PUM_TRX_ID', 'a.PUM_TRX_ID')
                ->where('a.TRX_DATE', $data->TRX_DATE)
                ->where('a.ORG_ID', $org_id)
                ->where('a.CREATED_BY', $user_id)
                ->whereBetween('a.TRX_DATE', [$create_start_date, $create_end_date])
                ->whereBetween('a.FINAL_DATE', [$validate_start_date, $validate_end_date])
    ...
    public function responsePum($user_id, $dept_id, $create_start_date, $create_end_date, $pum_status, $resp_status, $validate_start_date, $validate_end_date, $org_id){
        $getEmp = DB::connection('api_sys')->table('sys_user')->select('EMP_ID')->where('USER_ID', $user_id)->get()->toArray();
        ...
        $data   = DB::connection('api_pum')->table('pum_resp_trx_all as a')
            ...
            ->where('c.EMP_ID', $getEmp[0]->EMP_ID)
